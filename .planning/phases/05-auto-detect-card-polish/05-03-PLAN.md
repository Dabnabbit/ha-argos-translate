...
must_haves:
  ...
  key_links:
    - from: "custom_components/argos_translate/frontend/argos_translate-card.js"
      ...
      pattern: 'source.*auto'
    - from: "custom_components/argos_translate/frontend/argos_translate-card.js"
      to: "argos_translate.detect service"
      via: "after auto-detect translation, calls detect service to get candidate languages for dropdown"
      pattern: "argos_translate.*detect"
    - from: "custom_components/argos_translate/frontend/argos_translate-card.js"
      to: "argos_translate.translate service"
...
```
  ...
  <verify>
```bash
node --check custom_components/argos_translate/frontend/argos_translate-card.js
grep -q '"auto"' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q '_detectionCandidates' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'DETECTION_CONFIDENCE_THRESHOLD' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'auto:' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q '_getLanguageName' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q '_getTargetsForSource' custom_components/argos_translate/frontend/argos_translate-card.js
```
  </verify>
  <done>
...
```bash
node --check custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'detected_language' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'detection_confidence' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'detection-info' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'detect.*text' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'DETECTION_CONFIDENCE_THRESHOLD' custom_components/argos_translate/frontend/argos_translate-card.js
grep -q 'uninstalled_detected_language' custom_components/argos_translate/frontend/argos_translate-card.js
```
  </verify>
  <done>
After auto-detect translation, source dropdown updates to "Auto (French)" showing detected language. Detection info line displays below output with language name and confidence percentage. Uninstalled detected language shows a warning message. Detection candidates are fetched via the detect HA service and populated in the source dropdown as selectable "Auto (Language)" options above the confidence threshold. Selecting a candidate re-translates with that fixed source.
  </done>
</task>
