...
If Plan 02 set it to "0.4.0", bump to "0.5.0" to reflect auto-detect additions.
  ...
  <verify>
Review the card JS for: (1) `_translate` reads `detected_language` and `detection_confidence` from service response, (2) `_detectedLanguage` property updates after auto-detect translation, (3) detection info line shows "Detected: French (92%)" below output, (4) source dropdown label changes to "Auto (French)" when detection is set, (5) uninstalled language warning displays as error message, (6) `_translate` calls `argos_translate.detect` service after auto-detect and populates `_detectionCandidates` from response filtered by `DETECTION_CONFIDENCE_THRESHOLD`.
  </verify>
  <done>
