...
Bump `CARD_VERSION` to `"0.4.0"`.
  ...
  <verify>
Open the card JS file and confirm: (1) all `<select>` elements have `aria-label`, (2) both `<textarea>` elements have `aria-label`, (3) swap button has `aria-label`, (4) the catch block checks `err.code`, (5) `_getDisabledReason()` method exists and hint is rendered. Check syntax by loading the file in a browser dev console or reviewing manually.
  </verify>
  <done>
...
```
  ...
  <verify>
Visually inspect the card JS file for: (1) `:host` has `container-type: inline-size`, (2) `.content-area` div wraps input/output panels, (3) `@container argos-card (min-width: 580px)` rule exists, (4) `data-layout` attribute set on `:host` from config, (5) override CSS for horizontal/vertical exists, (6) `.lang-row` has `flex-wrap: wrap`, (7) card editor has Layout selector. Check card loads without JS errors by reviewing syntax.
  </verify>
  <done>
